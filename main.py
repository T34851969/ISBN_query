...
class Aplication:
    ...
    def _build_init_ui(self) -> None:
        ...
        with ui.scroll_area(on_scroll=lambda: scroll_area.scroll_to(percent=1.00)) as scroll_area:
            self.text = ui.markdown('').style("white-space: pre-wrap; font-size:16px;")
            self.text.set_content(Global_logger.return_logs())
            # 定时刷新日志内容
            ui.timer(0.5, lambda: self.text.set_content(Global_logger.return_logs()))
    ...
    def _build_main_ui(self) -> None:
        ...
        with ui.tab_panels(main_tabs, value=tab1):
            with ui.tab_panel(tab1):
                ui.markdown('## **单次**')
                text_in_single = ui.input('请输入 ISBN，可带连字符或空格：', placeholder='978-7-111-12345-6').style(
                    "width:400px;")
                ui.button('查询', on_click=lambda: self._on_single_check(text_in_single.value)).style(
            ...
            with ui.tab_panel(tab2):
                ...
                with ui.tab_panels(sub_tabs, value=sub_tab1):
                    with ui.tab_panel(sub_tab1):
                        ...
                        ui.button('查询', on_click=lambda _: self._on_batch_check(batch_textarea.value)).style(
